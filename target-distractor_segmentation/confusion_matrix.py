...
def pretty_plot_confusion_matrix(result_dir, df_cm, annot=True, cmap="Oranges", fmt='.2f', fz=11,
...
def plot_confusion_matrix_from_data(result_dir, classification, y_test, predictions, columns=None, annot=True, cmap="Oranges",
    ...
    if(not columns):
        if int(classification)==2:
            class_list = ['Background', "Target", "Distractor"]
        elif int(classification)==3:
            class_list = ['Background', "Target", "Low distractor", "High distractor"]
        columns = ['%s' %(i) for i in list(class_list)[0:max(len(np.unique(y_test)),len(np.unique(predictions)))]]
    ...
    pretty_plot_confusion_matrix(result_dir, df_cm, fz=fz, cmap=cmap, figsize=figsize, show_null_values=show_null_values, 
